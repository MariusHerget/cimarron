...
void paint(std::vector<keypoint_trajectory> &trs,
  ...
  // Decrease opacity.
  pixel_wise(paint_buffer) | [](vuchar4 &v) { v[3] *= 0.97; };
...
#pragma omp parallel for
  for (int i = 0; i < trs.size(); i++) {
    ...
    if (t.alive() and t.size() > 1) {
      ...
      vint2 p3 = t.position_at_frame(t.end_frame() - std::min(t.size() - 1, 10))
                     .template cast<int>();

      float speed = (p3 - p1).norm();
      vuchar3 pt_color = hsv_to_rgb(
          (M_PI + atan2(p3[0] - p1[0], p3[1] - p1[1])) * 180.f / M_PI, 1.f,
          1.f);

      float alpha = std::min(1.f, speed / 10);

      auto paint = [&](vint2 x, float a) {
        if (!paint_buffer.has(x))
          return;
        vuchar4 c;
...
int main(int argc, const char *argv[]) {
  ...
  if (opts.record_video.size()) {
    output_video.open(opts.record_video,
                      cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 30.f,
                      cv::Size(domain.ncols(), domain.nrows()), true);
  }

  int us_cpt = 0;
  int totalFrame = 0;
  foreach_videoframe(opts.video.c_str()) |
      [&](const image2d<vuchar3> &frame_cv) { totalFrame++; };
  foreach_videoframe(opts.video.c_str()) |
      [&](const image2d<vuchar3> &frame_cv) {
        ...
        t.start();
        // if (!first)
        //   video_extruder_update(
        //       ctx, prev_frame, frame_graylevel, _detector_th =
        //       opts.detector_th, _keypoint_spacing = opts.keypoint_spacing,
        //       _detector_period = 1, _nscales = 3, _winsize = 9, _patchsize =
        //       5, _propagation = 2);
        // else
        //   first = false;
        ...
        vpp::copy(frame_graylevel, prev_frame);
        // auto display = clone(frame);
        // paint(ctx.trajectories, paint_buffer);

        // pixel_wise(paint_buffer, display) | [](auto p, auto &d) {
        //   d = ((d.template cast<int>() * (255 - p[3]) +
        //         p.template segment<3>(0).template cast<int>() * p[3]) /
        //        255)
        //           .template cast<unsigned char>();
        // };
